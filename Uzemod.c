...
#include <spiram.h>
#include <bootlib.h>
//#include "../../gfx/fonts/8x8/font-8x8-full.inc"
#include "data/tiles.inc"
#include "Uzemod.h"



void RecalculateWaveform(Oscillator_t *oscillator){
	s32 result = 0;
	...
	if(mp.tick == 0){
		...
		for(u8 ch=0; ch<MOD_CHANNELS; ch++){
			...
			if(eff_tmp || effval_tmp){
				switch(eff_tmp){
					case 0x3:
						if(effval_tmp) mp.ch[ch].slideamount = effval_tmp;
						// fallthrough
					case 0x5:
						mp.ch[ch].slidenote = mp.ch[ch].note;
						break;
					case 0x4:
						if(effval_tmp & 0xF0) mp.ch[ch].vibrato.speed = effval_tmp >> 4;
						if(effval_tmp & 0x0F) mp.ch[ch].vibrato.depth = effval_tmp & 0x0F;
						// fallthrough
					case 0x6:
						RecalculateWaveform(&mp.ch[ch].vibrato);
						break;
					case 0x7:
						if(effval_tmp & 0xF0) mp.ch[ch].tremolo.speed = effval_tmp >> 4;
						if(effval_tmp & 0x0F) mp.ch[ch].tremolo.depth = effval_tmp & 0x0F;
					...
					case 0xD:
						if(mp.skiporderrequest < 0){
							if(mp.order + 1 < mp.orders) mp.skiporderrequest = mp.order + 1;
							else mp.skiporderrequest = 0;
						}
					...
					case 0xE:
						switch(effval_tmp >> 4){
							case 0x1: mp.ch[ch].period -= (effval_tmp & 0xF); break;
							case 0x2: mp.ch[ch].period += (effval_tmp & 0xF); break;
							case 0x4: mp.ch[ch].vibrato.waveform = (effval_tmp & 0x7); break;
							case 0x6:
								if(effval_tmp & 0xF){
									if(!mp.patloopcycle) mp.patloopcycle = (effval_tmp & 0xF) + 1;
								...
								break;
							case 0x7: mp.ch[ch].tremolo.waveform = effval_tmp & 0x7; break;
							case 0xA:
								mp.ch[ch].volume += effval_tmp & 0xF;
								if(mp.ch[ch].volume > 0x40) mp.ch[ch].volume = 0x40;
								break;
							case 0xB:
								mp.ch[ch].volume -= effval_tmp & 0xF;
								if(mp.ch[ch].volume < 0) mp.ch[ch].volume = 0;
								break;
							case 0xE:
	...
	for(u8 ch=0; ch<MOD_CHANNELS; ch++){
		...
		if(eff_tmp || effval_tmp){
			switch(eff_tmp){
				case 0x0:
					switch(mp.tick % 3){
						case 0: mp.ch[ch].period = mp.ch[ch].note; break;
						case 1:
							mp.ch[ch].period = (mp.ch[ch].note *
								pgm_read_dword_near(&arpeggio_table[effval_tmp >> 4])) >> 16;
							break;
						case 2:
							mp.ch[ch].period = (mp.ch[ch].note *
								pgm_read_dword_near(&arpeggio_table[effval_tmp & 0xF])) >> 16;
							break;
					}
					break;
				case 0x1:
					if(mp.tick) mp.ch[ch].period -= effval_tmp;
					break;
				case 0x2:
					if(mp.tick) mp.ch[ch].period += effval_tmp;
					break;
				...
				case 0x3:
					if(mp.tick){
						if(!effval_tmp) effval_tmp = mp.ch[ch].slideamount;
		...
		if(mp.ch[ch].period < 0) mp.ch[ch].period = 0;
		if(mp.ch[ch].period){
...
*/
	...
	for(u16 s=0; s<span_len; s++){
		if(localCurrPtr >= localLength){ //if at or past end of non-loop sample, deactive and stop
			localVolume = 0;
		...
		//read sample byte
		u32 desired = localSpiBase + localCurrPtr;
...
while(localCurrPtr >= localLength) {
	...
	pch->volume		= localVolume;
		...
		if(masterVolume == 64){
			...
			for(u16 s=0; s<span_len; s++){
				s32 scaled = (accum_span[s] * (int32_t)masterVolume) >> 14; // 14 = 8+6
...
u8 InitMOD(){	
	...
	for(u8 i=0; i<MAX_SAMPLES; i++){ //scan sample headers to determine total file size
		...
		SpiRamSeqReadEnd();
		if(sample_data_last_byte >= detected_ram){
	...
	for(u16 i=3; i<total_sectors; i++){ //load remaining sectors into SPI RAM
		...
		FS_Read_Sector(&sd);
		FS_Next_Sector(&sd);
		u32 addr32 = MOD_BASE + (u32)i * 512UL;
		SpiRamSeqWriteStart((u8)(addr32 >> 16), (u16)(addr32 & 0xFFFF));
		for(u16 j=0; j<512; j++){
			SpiRamSeqWriteU8(sd.bufp[j]);
		}
		SpiRamSeqReadEnd();
...
int main(){
	...
	ClearVram();
	DrawWindow(1, 1, 28,10);
	DDRC = 0;
	WaitVsync(1);
	LoadPreferences();

	sd.bufp = &(accum_span[0]);
	u8 res = FS_Init(&sd);
	if(res != 0U){
	...
	if(!SpiRamInit()){
		...
		u8 bank_count;
		SpiRamWriteU8(0,0,0xFF);
		for(bank_count=0; bank_count<128; bank_count++){
			SpiRamSeqWriteStart(bank_count, 0xFFFF);
			SpiRamSeqReadStart(bank_count, 0xFFFF);
			SpiRamSeqReadEnd();
			SpiRamSeqWriteEnd();
			if(SpiRamReadU8(0,0)==0) break;
		}
		bank_count++;
		detected_ram = (u32)(bank_count*(64UL*1024UL));
		if(bank_count < 9){
			PrintLong(21,cony,(u32)bank_count*64);
			UMPrintChar(22,cony,'K');
		}else{
			PrintLong(22,cony,(u32)bank_count/8);
			UMPrintChar(19,cony,'M');
		}
		UMPrint(3,cony++,PSTR("SPI RAM Detected:"));
	}
	u32 file_pos = FS_Find(&sd,
		((u16)('M')<<8)|('O'),
		((u16)('D')<<8)|('_'),
		((u16)('D')<<8)|('A'),
		((u16)('T')<<8)|('A'),
	...
	if(file_pos == 0U){
		...
		FS_Select_Cluster(&sd, file_pos);
		UMPrint(3,cony++,PSTR("Found: MOD_DATA.MOD"));
	}
	InitMOD();
	ptime_min = ptime_sec = ptime_frame;
	DrawMap(5,0,buttons_map);
	PrintSongTitle((CONT_BAR_X/8),(CONT_BAR_Y/8)+2);
	sprites[0].tileIndex = TILE_CURSOR;
	sprites[0].flags = sprites[0].x = sprites[0].y = 0;
//	UMPrint(1,0,PSTR("LOAD"));
	while(1){
		WaitVsync(1);
		SetRenderingParameters(33, 1);
//		UMPrintChar(0,0,'-');
//		PrintInt(4,1,spi_seeks,1);
	UMPrint(PTIME_X,PTIME_Y,PSTR("  :  :  "));
//		spi_seeks = 0;
		RenderMOD(fast_forward);
//		UMPrintChar(0,0,'+');
PrintByte(2,0,DDRC,1);
PrintByte(2,1,masterVolume,1);
	UserInterface();
...
void UartSendStringFlash(const char *s){
	while(1){
...
void UartSendStringSpiRam(u32 soff){
	SpiRamSeqReadStart((u8)(soff>>16),(u16)(soff&0xFFFF));
	while(1){
...
u8 DownloadFile(u8 entry){
	InitUartRxBuffer();
	InitUartTxBuffer();
	UartSendStringFlash(PSTR("AT+CIPSTART="));
	u32 eoff = (u32)entry*256UL;
	UartSendStringSpiRam(eoff);
}

u8 SendQuery(){//response will be stored starting at offset 0
	
}

void DebugDump(u32 off){
	SpiRamSeqReadEnd();
	SpiRamSeqReadStart((u8)(off>>16),(u16)(off&0xFFFF));
	for(u8 y=0; y<SCREEN_TILES_V; y++){
		for(u8 x=0; x<SCREEN_TILES_H; x++){
			PrintHexByte(x*2,y,SpiRamSeqReadU8());
		}
	}
	SpiRamSeqReadEnd();
	while(1);
...
void InputDeviceHandler(){//requires the kernel to *NOT* read controllers during VSYNC
	u8 i;
	joypad1_status_lo = 0;
	joypad2_status_lo = 0;

	JOYPAD_OUT_PORT |= _BV(JOYPAD_LATCH_PIN);//latch controllers
	for(i=0;i<8;i++)//7 seems to glitch
		Wait200ns();
	...
	for(i=0; i<16; i++){//read button states from the shift registers 
		...
		for(uint8_t j=0;j<33;j++)//32 seems to glitch?
			Wait200ns();
	}
	...
	for(i=0; i<8; i++)//wait 1.6us, any less and it glitches
		Wait200ns();
...
static void UserInterface(){
	...
	if((pad & BTN_UP)){
		...
		if(sprites[0].y+8+speed > ylimit+3)
			sprites[0].y = sprites[0].y = ylimit-8+3;
		else
			sprites[0].y += speed;
	}
	...
	for(u8 i=0; i<2; i++){
		...
		if(i == 0){
			...
			xsign = (joypad1_status_hi & 0b1000000000000000)>>8;
			ysign = (joypad1_status_hi & 0b0000000010000000);
		}else{
			deltax = (joypad2_status_hi & 0b0000000001111111);
			deltay = (joypad2_status_hi & 0b0111111100000000)>>8;
			xsign = (joypad2_status_hi & 0b1000000000000000)>>8;
			ysign = (joypad2_status_hi & 0b0000000010000000);
		}
		if(xsign){ //left mouse movement
			deltax = -deltax;
			deltax += sprites[0].x;
			sprites[0].x = (deltax < 0)?0:deltax;
		}else{ //right mouse movement
			deltax += sprites[0].x;
			sprites[0].x = (deltax > (SCREEN_TILES_H*TILE_WIDTH)-8)?(SCREEN_TILES_H*TILE_WIDTH)-8:deltax;
		}
		if(ysign){ //up mouse movement
			deltay = -deltay;
			deltay += sprites[0].y;
			sprites[0].y = (deltay < 0)?0:deltay;
		}else{ //down mouse movement
			deltay += sprites[0].y;
			sprites[0].y = (deltay > ylimit-4)?(ylimit-4):deltay;
		}
	}
	...
	if(sprites[0].y >= (CONT_BAR_Y+CONT_BTN_H) || sprites[0].x < CONT_BAR_X || sprites[0].x >= CONT_BAR_X+CONT_BAR_W)
		...
		/*if(btn > 6){ //half height buttons..
			if(sprites[0].y > 8)
				btn = 255;
		}*/
	}
	static u8 lastbtn = 255;
	if((pad & BTN_Y) && !(oldpad & BTN_Y)){
		lastbtn = btn;
		newclick = 1;
	}
	...
	if(newclick){
		if(btn == 0){//previous
			...
			play_state = 0;//eliminate pause if present
		}else if(btn == 5){//next
...
static u8 SavePreferences(){
	...
	//	ebs.data[28] = 64;//default master volume
		ebs.data[29] = DDRC;//DEFAULT_COLOR_MASK;
